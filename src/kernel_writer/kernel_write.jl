...
function kgen(num::Num, gradlist::Vector{Num}, raw_outputs::Vector{Symbol}, constants::Vector{Num}, overwrite::Bool, splitting::Symbol, affine_quadratic::Bool)
    ...
    all_vars = [Symbol.(get_name.(gradlist))..., get_name.(getfield.(factored, :lhs))...]
    sparsity = get_sparsity(all_vars, Symbol.(get_name.(gradlist)), factored)
    ...
    if (n_vars[end] < 31) && ((n_lines[end] <= max_size) || (findfirst(x -> x > split_point, n_lines)==length(n_lines)))
        ...
        create_kernel!(expr_hash, 1, factored, length(factored), get_name.(gradlist), constants, all_vars, sparsity)
        push!(kernel_nums, 1)
        ...
        while !complete
            ...
            create_kernel!(expr_hash, kernel_count, factored, new_ID, get_name.(gradlist), constants, all_vars, sparsity)
            ...
            if (n_vars[end] < 32) && (all(n_lines[1:end-1] .<= max_size))
                create_kernel!(expr_hash, kernel_count, factored, length(factored), get_name.(gradlist), constants, all_vars, sparsity)
                push!(kernel_nums, kernel_count)
...
function create_kernel!(expr_hash::String, kernel_ID::Int, factored::Vector{Equation}, stop_point::Int, gradlist::Vector{Symbol}, constants::Vector{Num}, all_vars::Vector{Symbol}, sparsity::Vector{Vector{Int}})
    ...
    # "f_" * expr_hash * "_$n". 
    # This name will be pushed to `kernels`, and a vector of the required 
    # inputs variables will be pushed to `inputs`. Other inputs are needed 
    # to determine portions of the kernel itself.

    # Start by getting a smaller version of `factored` that only contains
    # factors relevant to the `stop_point`
    short_factored = shorten(factored, stop_point)

    # Collect vars that are needed as inputs (this may be base-level variables,
    # auxiliary variables, or a combination)
    input_vars = pull_vars(extract(factored, stop_point), get_names=true)

    # Put the factorized expression into directed acyclic graph form
    edgelist = eqn_edges(short_factored, all_vars)
    ...
    varorder = all_vars[topological_sort(g)]

    # Glossary:
    # all_vars: ALL variables including base variables and aux variables
    # input_vars: Variables needed as inputs to this expression
    # varorder: A topologically sorted list of variables (i.e., in calculation order)
    # gradlist: Non-auxiliary variables
    # g.fadjlist[i]: Contains indices of all_vars that depend on all_vars[i]
    # g.badjlist[i]: Contains indices of all_vars that are needed to compute all_vars[i]

    # Calculate the number of temporary variables needed. 
    temp_endlist = []
    maxtemp = 0
    for i in eachindex(varorder) # Loop through every participating variable
        if (varorder[i] in input_vars)
            # Skip the variable if it's an input
            continue
        end
        # Find which index varorder[i] is in `all_vars`
        ID = findfirst(==(varorder[i]), all_vars)
        tempID = 0
        ...
        if isempty(temp_endlist)
            ...
            # in varorder.
            factored_ID = findfirst(==(varorder[i]), get_name.(getfield.(factored, :lhs)))
            if exprtype(factored[factored_ID].rhs) == ADD
    ...
    # Put in the preamble.
    write(file, preamble_string(expr_hash, ["OUT"; string.(input_vars)], kernel_ID, maxtemp, length(gradlist)))

    # Loop through the topological list to add calculations in order
    temp_endlist = []
    outvar = ""
    name_tracker = copy(all_vars)
    for i in eachindex(varorder) # Order in which variables are calculated
        ...
        # Determine the index of the variable in `all_vars`
        ID = findfirst(==(varorder[i]), all_vars)

        # Determine which element of the (full) factorization we're using
        factored_ID = findfirst(==(varorder[i]), get_name.(getfield.(factored, :lhs)))

        # Determine the indices of the inputs to this calculation
        inputs = []
        input_IDs = Int[]
        participants = pull_vars(factored[factored_ID], get_names=true)
        for p in participants
            p_ID = findfirst(==(p), all_vars)
            push!(inputs, string(name_tracker[p_ID]))
            push!(input_IDs, p_ID)
        end
        ...
        if length(inputs)>2 && inputs[1]==inputs[2]
            ...
            write_operation(file, factored[factored_ID].rhs, inputs, string.(gradlist), sparsity[input_IDs[3]])
...
function get_sparsity(all_vars, symbol_gradlist, factored)
    sparsity = [Int[] for _ in 1:length(all_vars)]
    function calc_sp(v)
        if !isempty(sparsity[v])
            return sparsity[v]
        end
        if all_vars[v] in symbol_gradlist
            sparsity[v] = [findfirst(==(all_vars[v]), symbol_gradlist)]
            return sparsity[v]
        end
        idx = findfirst(x -> isequal(Symbol(x.lhs), all_vars[v]), factored)
        RHS_vars = Symbol.(get_name.(pull_vars(factored[idx].rhs)))
        for var in RHS_vars
            var_idx = findfirst(==(var), all_vars)
